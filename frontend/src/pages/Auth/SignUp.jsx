  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const errors = response.response?.data?.message?.errors;
        if (errors?.email) {
            ...
            <form className="space-y-6">
              ...
              >
                <div className="relative group">
                  <UserRound className="absolute left-3 top-3 w-5 h-5 text-neutral-400 group-hover:text-secondary transition-colors" />
                  <Input
                    type="text"
