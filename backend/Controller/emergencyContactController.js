...
exports.deleteEmergencyContact = async (req, res) => {
  ...
  try {
    // ✅ ownership check — only delete if contact belongs to this user
    ...
    await logActivity({
      ...
      description: `${deleted.fullname} was removed from emergency contacts`,
    });
...
exports.editEmergencyContact = async (req, res) => {
  const { contactId }              = req.params;
  const { userId }                 = req.payload; // ✅ correctly extracted
  const { fullname, phoneNumber, email } = req.body;

  try {
    // ✅ ownership check — only edit if contact belongs to this user
    ...
    // ✅ activity log now added for edit
    await logActivity({
      userId,
      type:        "contact_updated",
      title:       "Emergency Contact Updated",
      description: `${updatedContact.fullname}'s details were updated`,
    });
  ...
  <div style="font-family: Arial, sans-serif; max-width: 520px; margin: 0 auto;
    ...
    <div style="background: #3d3b3b; padding: 16px 32px; text-align: center;
      ...
      <span style="display: inline-block; background: rgba(234,43,31,0.18);
                   ...
                   color: #EA2B1F; letter-spacing: 0.8px;">🔔 TEST ALERT</span>
    </div>
    <div style="padding: 28px 32px;">
      ...
      <div style="background: #3d3b3b; border: 1px solid rgba(255,255,255,0.08);
        ...
        <table style="width: 100%; border-collapse: collapse;">
          <tr>
            <td style="padding: 5px 0; font-size: 13px; color: rgba(255,255,255,0.45); width: 40%;">📋 Status</td>
            <td style="padding: 5px 0; font-size: 13px; font-weight: 700; color: #4ade80;">Test — No action required</td>
          </tr>
          <tr>
            <td style="padding: 5px 0; font-size: 13px; color: rgba(255,255,255,0.45);">🕐 Sent at</td>
            <td style="padding: 5px 0; font-size: 13px; font-weight: 600; color: #FFFFFF;">
              ${new Date().toLocaleString('en-IN', { dateStyle: 'medium', timeStyle: 'short' })}
            </td>
          </tr>
          <tr>
            <td style="padding: 5px 0; font-size: 13px; color: rgba(255,255,255,0.45);">👤 Sent to</td>
            <td style="padding: 5px 0; font-size: 13px; font-weight: 600; color: #FFFFFF;">${contact.fullname}</td>
          </tr>
        </table>
      </div>
